...
<body>
  ...
  <div class="content-wrapper">
        ...
        <div class="row">
          <div class="box">
            <div class="box-header with-border">
              <h4 class="box-title">Collect Admission Fees</h4>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            ...
            if (isset($_POST['submit'])) {
              ...
              if (mysqli_num_rows($check_student)==0) {
                echo "<script>alert('Invalid Admission Number'),window.open('collect_adm_fees.php','_self')</script>";
                die();
              }
              $months = array(4, 5, 6);
              ...
              $admission_fees_details = mysqli_query($con,"SELECT * FROM tb_admission_fees WHERE class_id='$class_id' AND sess_id='$session_id' ");

              $fees_amt = [];
              
              while ($all_adm_fees = mysqli_fetch_assoc($admission_fees_details)) {
                $fee_head = $all_adm_fees['fee_head'];
                $fee_amt = $all_adm_fees['fee_amt'];
                    ...
                    <div class="container" <?php echo $show_container ?>>
                      ...
                      if (isset($_POST['collect'])) {
                       ...
                       if ($final_amt != $amt_paid) {
                         echo "<script>alert('Amount Mismatched!'),window.open('collect_adm_fees.php','_self')</script>";
                          die();
                       }
                      ...
                      VALUES ('$stu_adm_no', '$month_id', '$class_id', '$session_id', '$tot_fee', '$amt_paid', '$pay_mode', '$bill_id', '$cash_amount', '$card_amount', '$upi_amount', '$due_fee', '$fee_date','Admission')";

                      if (mysqli_query($con, $sql)) {
                        $add_dues = mysqli_query($con,"UPDATE tb_students SET dues_amt = '$due_fee' WHERE adm_no='$stu_adm_no' ");
                        if ($add_dues) {
                          echo "<script>alert('Admission Fees Collected!'),window.open('adm_fee_receipt.php'),window.open('collect_adm_fees.php','_self')</script>";
                        }
    ...
    <script>
    function abc(abcaa,aaa,bb,class_id){                   
